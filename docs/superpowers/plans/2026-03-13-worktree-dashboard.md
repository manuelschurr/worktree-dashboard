...
**Goal:** Build a standalone Python TUI that shows all worktree-orchestrator sessions across multiple projects and lets you manage them with single keypresses.

**Architecture:** Single-file TUI (`tui.py`) using `rich` for rendering. Reads `sessions.json` from each configured project for display. Shells out to the bundled `orchestrator.py` for all mutations (spawn, restart, kill). Platform-specific keypress handling via `msvcrt` (Windows) or `tty`/`select` (Unix).

**Tech Stack:** Python 3.9+, `rich` (only external dependency), stdlib for everything else.

**Spec:** `docs/superpowers/specs/2026-03-13-orchestrator-tui-design.md`

**Orchestrator source (to bundle):** `C:\Users\schur\.claude\skills\worktree-orchestrator\scripts\orchestrator.py`

---

## Chunk 1: Scaffolding and bundled files

### Task 1: Bundle orchestrator.py and create templates

**Files:**
- Create: `orchestrator.py` (copy from skill)
- Create: `templates/orchestrator.toml`
- Create: `templates/secrets`
- Create: `requirements.txt`
- Create: `.gitignore`

- [ ] **Step 1: Copy orchestrator.py from the Claude Code skill**

Copy `C:\Users\schur\.claude\skills\worktree-orchestrator\scripts\orchestrator.py` to `~/code/worktree-dashboard/orchestrator.py`. This is a verbatim copy — no modifications.

```bash
cp "C:/Users/schur/.claude/skills/worktree-orchestrator/scripts/orchestrator.py" ~/code/worktree-dashboard/orchestrator.py
```

- [ ] **Step 2: Create templates/orchestrator.toml**

This is the example config users copy into their repos. Based on the template generated by `orchestrator.py init`, but cleaned up with clearer comments.

```toml
# .orchestrator.toml — copy this to your repo root and edit.
#
# Secrets (DATABASE_URL, API keys) go in .orchestrator/.secrets, NOT here.
...
# Define one [servers.<name>] section per process.
# Ports are auto-assigned. Use these placeholders:
#   {port}              — this server's own port
#   {backend.port}      — the "backend" server's port
#   {frontend.port}     — the "frontend" server's port
#
# Commands must run non-interactively (no TTY).
...
- [ ] **Step 3: Create templates/secrets**

```
# .orchestrator/.secrets — copy this to <your-repo>/.orchestrator/.secrets
#
# Loaded into every server's environment automatically.
...
```bash
git add orchestrator.py templates/ requirements.txt .gitignore
git commit -m "feat: bundle orchestrator.py and add project templates"
```

---

### Task 2: Create the TUI skeleton with config loading and TOML parsing

**Files:**
- Create: `tui.py`

This task creates the entry point with config loading, orchestrator script discovery, and the TOML parser (reused from orchestrator.py since we can't import it — it uses `sys.exit` in its helpers).

- [ ] **Step 1: Write tui.py with imports, TOML parser, and config loading**
...
Create a test config and verify loading works:

```bash
mkdir -p ~/.config
# Create config with at least one project that has .orchestrator/
python tui.py
```

Expected: prints orchestrator path and project names, no errors.

- [ ] **Step 3: Commit**

```bash
git add tui.py
git commit -m "feat: TUI skeleton with config loading and TOML parser"
```

---

## Chunk 2: Data loading, health checks, and rendering

### Task 3: Session data loading and health checks

**Files:**
- Modify: `tui.py`

Add functions to load sessions from each project's `.orchestrator/sessions.json` and check PID liveness.

- [ ] **Step 1: Add session loading and health check functions to tui.py**

Add after the config loading section:
...
- [ ] **Step 2: Test by printing dashboard data**

Temporarily update `main()` to call `build_dashboard_data` and print the result:
...
def main():
    ...
    for proj in data:
        ...
        for s in proj["sessions"]:
            srvs = "  ".join(f"{sv['name']}:{'UP' if sv['alive'] else 'DOWN'}" for sv in s["servers"])
            print(f"  {s['key']}  {s['branch']}  {srvs}  {s['status']}")
```

Run `python tui.py`. Expected: sees live sessions from waidplan with correct UP/DOWN status.

- [ ] **Step 3: Commit**

```bash
git add tui.py
git commit -m "feat: session data loading and PID health checks"
```

---

### Task 4: Rich rendering

**Files:**
- Modify: `tui.py`

Add the dashboard rendering function using `rich`.
...
def render_dashboard(data: list[dict], selected_idx: int, selectable_items: list[dict], auto_refresh: bool):
    ...
    # Footer
    console.print("\n" + "─" * console.width)
    console.print("[dim]↑↓/jk[/dim] navigate  [dim]r[/dim] restart  [dim]x[/dim] kill  [dim]X[/dim] kill+remove  [dim]s[/dim] spawn  [dim]l[/dim] logs  [dim]a[/dim] auto-refresh  [dim]q[/dim] quit")
```

- [ ] **Step 2: Build the selectable items list**

Add a helper that flattens the dashboard data into a list of selectable session items (skipping project headers):
...
- [ ] **Step 3: Test rendering with dummy main loop**

Update `main()` to render once:
...
def main():
    ...
    render_dashboard(data, 0, items, True)
```

Run `python tui.py`. Expected: a nicely formatted dashboard with colored status indicators.

- [ ] **Step 4: Commit**

```bash
git add tui.py
git commit -m "feat: rich dashboard rendering with server health indicators"
```

---

## Chunk 3: Input handling and main loop

### Task 5: Platform-specific keypress reading

**Files:**
- Modify: `tui.py`

- [ ] **Step 1: Add get_key function**

Add after the imports section. This handles both Windows (`msvcrt`) and Unix (`tty`/`select`), including arrow key decoding:
...
```bash
git add tui.py
git commit -m "feat: platform-specific keypress handling (Windows + Unix)"
```

---

### Task 6: Main event loop with navigation
...
Replace the placeholder `main()` with the full event loop:
...
def main():
    ...
    try:
        while True:
            ...
            # Actions (implemented in next task)
            elif key in ('r', 'ENTER'):
                if items:
...
- [ ] **Step 2: Add placeholder action functions**

These will be implemented in the next task. For now, they're stubs so the loop compiles:

```python
def do_restart(orch_script, session): pass
def do_kill(orch_script, session): pass
def do_kill_remove(orch_script, session): pass
def do_spawn(orch_script, data, items, selected_idx): pass
def do_logs(orch_script, session): pass
```

- [ ] **Step 3: Test the loop**

Run `python tui.py`. Expected: dashboard renders, arrow keys/j/k move the cursor, `a` toggles auto-refresh indicator, `q` exits cleanly. Action keys are no-ops for now.

- [ ] **Step 4: Commit**

```bash
git add tui.py
git commit -m "feat: main event loop with navigation and auto-refresh"
```
...
### Task 7: Implement all actions (restart, kill, spawn, logs)

**Files:**
- Modify: `tui.py`

- [ ] **Step 1: Implement the orchestrator subprocess helpers**

Two variants: `run_orchestrator` captures output (for restart/kill/logs where output is short), and `run_orchestrator_live` streams output to the terminal in real-time (for spawn which can take 10-30 seconds):
...
Also add a helper for single-keypress confirmation that works on both platforms:
...
- [ ] **Step 3: Implement do_kill and do_kill_remove**
...
- [ ] **Step 6: Remove the placeholder stubs**

Delete the earlier `def do_restart(...): pass` etc. stubs, since they're now fully implemented.

- [ ] **Step 7: Test each action**

With live sessions running in waidplan:
1. Navigate to a session, press `l` — should show logs, return on keypress
2. Press `r` on a running session — should confirm, restart, show output
3. Press `s` — should prompt for name, spawn (test with `--no-claude`)
4. Press `x` on a session — should confirm, kill
...
```bash
git add tui.py
git commit -m "feat: implement all actions (restart, kill, spawn, logs)"
```

---

## Chunk 5: Polish and finalize
...
- [ ] **Step 1: Add init command support**

Add `i` keybinding for initializing orchestrator in a project. This runs `orchestrator.py init` in the selected project's directory:

```python
# In the keybindings section of the main loop, add:
elif key == 'i':
...
- [ ] **Step 2: Update footer to include init keybinding**

In the `render_dashboard` function, update the footer line:
...
1. Run `python tui.py` with live sessions
2. Verify auto-refresh updates health status
3. Test all keybindings: navigation, restart, kill, kill+remove, spawn, logs, init
4. Test `q` exits cleanly
5. Test with no sessions (should show "(no sessions)" per project)
6. Test Ctrl+C exits gracefully

- [ ] **Step 4: Commit**

```bash
git add tui.py
git commit -m "feat: add init command and polish"
```

---

### Task 9: Create user config for the current machine

**Files:**
- Create: `~/.config/worktree-dashboard.toml`

- [ ] **Step 1: Create the dashboard config**
...
```bash
cd ~/code/worktree-dashboard
python tui.py
```

Expected: Dashboard shows waidplan sessions with live health status, all keybindings work.

- [ ] **Step 3: Final commit with any remaining fixes**

```bash
git add -A
git commit -m "chore: finalize worktree-dashboard v1"
```
